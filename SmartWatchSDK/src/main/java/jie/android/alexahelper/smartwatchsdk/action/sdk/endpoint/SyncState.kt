...
//    if (endpoint != null) {
        postChangeReport(sdk, action, endpointId, changed, unchanged, callback)
//    } else {
...
private fun postChangeReport(sdk: SmartWatchSDK, action: ActionWrapper, endpointId: String, changed: JsonArray?, unchanged: JsonArray?, callback: ActionResultCallback) {
        ...
        changed?.let {
            addPayload("properties", buildJsonArray {
                for (index in 0 until  it.size) {
                    makeProperty(endpointId, it[index].jsonObject)?.let { it1 -> add(it1) }
                }
            })
        }
        unchanged?.let {
            setContext(buildJsonObject {
                put("properties", buildJsonArray {
                    for (index in 0 until unchanged.size) {
                        makeProperty(endpointId, unchanged[index].jsonObject)?.let { it1 -> add(it1) }
                    }
                })
            })
        }
